...
"model2d_9_plots" <- function(fitsum, L, source=9) {
	...
	pdf(paste0("pdf/2d_9/corr_L",L,"_",WHICH_CDAT,"_source",source,".pdf"))
		par(bty="l")
		plot(uf,corrs[[1]],
			...
			type="l",lwd=2,col="blue",ylim=c(-0.5,1), xaxt='n',xlim=rev(range(uf)))   # BC
	...
	if (source == 8 || source == 9) {
		...
		pdf(paste0("pdf/2d_9/cond_corr_L",L,"_",WHICH_CDAT,"_source",source,".pdf"))
			par(bty="l")
			plot(uf,cond.corrs[1,],
				xlab="Grid Cells per Cycle",ylab="Conditional Correlation",
				main=paste0("Conditional Correlation given BC vs Grid Cells per Cycle\nL=",L,", DIC=",round(DIC,1),", pD=",round(pD,1)),
